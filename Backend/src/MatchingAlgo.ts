...
interface Order {
	id: string;
	pair: string; // Example: "BTC-USDT" "ETH-BTC : 1234"
	type: OrderType;
	price: number;
	quantity: number;
	...
	if (order.type === "BUY") {
		if (!buyOrders[order.pair])
			buyOrders[order.pair] = new Heap((a, b) => {
				if (a.price === b.price)
					return a.orderPlacedTime - b.orderPlacedTime;
		...
		for (const pair in orderMap) {
			...
			if (order) {
				...
				if (order.quantity === 0) {
					...
					console.log(
						`Order ID ${orderId} fully settled and removed.`
					);
				}
				updated = true;
				break;
...
function generateMatchingPairs(sendEvent: (data: any) => void) {
	...
	for (const pair in buyOrders) {
		...
		buyHeap
			.toArray()
			.forEach((order) => buyCopy.push(deepCopyOrder(order)));
		...
		sellHeap
			.toArray()
			.forEach((order) => sellCopy.push(deepCopyOrder(order)));
	...
	console.log("generatinggggggg", matchingPairs);
